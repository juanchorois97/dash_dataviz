...
### La app no carga datos (se queda en "Cargando datos WDI...")

**Causa probable:** Sin conexión a internet o API del Banco Mundial caída.

**Solución:** El pipeline tiene fallback automático. Si `wbgapi` falla, intenta con la REST API directa. Si ambos fallan, carga datos sintéticos. Verifica tu conexión:

```bash
curl "https://api.worldbank.org/v2/country/COL/indicator/SE.PRM.CMPT.FE.ZS?format=json&mrv=1"
```

---

### Error: `ModuleNotFoundError: No module named 'wbgapi'`
...
### Los mapas aparecen en blanco o sin colores

**Causa:** Plotly necesita conexión para cargar los datos geográficos la primera vez.

**Solución:** Asegurarse de tener conexión activa. Los mapas usan `locationmode="ISO-3"` que es nativo de Plotly y no depende de nombres de países.

---

### `df_2` tiene menos de 2,992 filas

Esto indica que la descarga fue parcial. Verificar en consola cuántas filas se obtuvieron:

```
[wbgapi] XXXX filas totales (con NAs), YYY países, ZZZ no-nulas
```

Si `ZZZ < 2992`, revisar la conexión o esperar y reiniciar. El número exacto de filas no nulas depende de los datos que el Banco Mundial tenga disponibles en ese momento.
...
```bash
# Cambiar el puerto en la última línea del archivo:
app.run(debug=False, host="0.0.0.0", port=8051)
```

---

### Gráficos lentos en el primer render

Normal: el primer renderizado de cada sección computa las estadísticas desde cero. Las siguientes navegaciones son instantáneas.
